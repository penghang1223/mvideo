...
public class VideoTypeDaoImpl extends BaseDao implements VideoTypeDao {
    ...
    public int updateType(VideoType videoType) {
        String sql = "UPDATE `VIDEOTYPE` SET `CATEGORY`=?,`SECCATEGORY`=?,`STATUS`=? WHERE `ID`=?";
        return update(sql,videoType.getCategory(),videoType.getSecCategory(),videoType.getStatus(),videoType.getId());
    }
    ...
    public VideoType queryVideoTypeBySecCategory(String category) {
        String sql ="`SELCET` `CATEGORY`,`SECCATEGORY`,`STATUS` FROM `VIDEOTYPE` WHERE `SECCATEGORY`=?";
        return queryForOne(VideoType.class,sql,category);
    }
}
